%{
  Titulo: Red de Jordan - Práctica de redes neuronales de predicción
...
function u0 = calculaU0(x, y1_anterior, w0)
	%{
		Funcion encargada de calcular el el vector u0 de la capa oculta
		
		x				    la entrada en la iteración actual (t)
		y1_anterior	la salida de la capa de salida en la iteración anterior (t-1)
...
function delta_w0 = calculaDeltaW0(
  ...
  for (i = 1:N1)
    ...
    for (j = 1:N0)
      ...
      delta_w0(j, NI+N1+1) = -ALPHA * delta0;                   % k = NI+N0+1
    end
  end
end
...
y1_anterior_2 = rand(1, N1) - 0.5 * ones(1, N1);
...
for (t = 1:size(entrenamiento,2))
  ...
  delta_w0 = calculaDeltaW0(x, y1_anterior, u1_anterior, w1, u0_anterior, x_anterior, y1_anterior_2);
